...
namespace ClassLibraryTest
{
	...
	/// <summary>
	///This is a test class for ClassLibrary.ChartCollection and is intended
	///to contain all ClassLibrary.ChartCollection Unit Tests
	///</summary>
	[TestFixture]
	public class ChartCollectionTest
	{
		/// <summary>
		...
		{
			ChartCollection target = new ChartCollection();

			string strOrderBy = "Position";

			target.Sort(strOrderBy);

			Assert.IsFalse(target.Count > 0, "ChartCollection is greater than 0.");
		}
	}

	/// <summary>
	///This is a test class for ClassLibrary.ChartSorter and is intended
	///to contain all ClassLibrary.ChartSorter Unit Tests
	///</summary>
	[TestFixture]
	public class ChartSorterTest
	{
		private readonly String m_strConnectionStringName = "appDatabase";

		/// <summary>
		///A test for Compare (Chart, Chart)
		///</summary>
		[Test]
		public void CompareTest()
		{
			...
			ChartSorter target = new ChartSorter(strOrderBy);
			Chart x = new Chart();
			Chart y = new Chart();
			int expected = 0;
			int actual;
			...
			while (reader.Read())
			{
				...
				Assert.AreEqual(expected, actual, "ClassLibrary.ChartSorter.Compare did not return the expected value.");
			}
