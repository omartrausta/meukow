...
namespace ClassLibrary
{
	public class ChartDoc : BaseDocument
	{
		...
		public DataSet GetChartList(int ID)
		{
			...
			String strSQL = string.Format("SELECT ListProp.[Position] AS Postition, Song.Name AS Song_Name, Artist.Name AS Artist_Name FROM ((Artist INNER JOIN Song ON Artist.ID = Song.ArtistID) INNER JOIN ListProp ON Song.ID = ListProp.Song) WHERE (ListProp.List = {0})", ID);
